# Day 3 success — reusable Codex prompt and response journal

This standalone coder-training copy was created on 2026-09-16 from `aithinking.md`. It preserves the available user-visible prompts, responses, decisions, evidence, and corrections so another coder can reuse the same governed methodology.

> Boundary: this is not private AI chain-of-thought. Hidden instructions, internal reasoning, credentials, and tool payloads are excluded. Historical responses unavailable verbatim are labelled as summaries.

---
## Source conversation and decision journal
...
### 19. Day 3 standalone coder copy

**User prompt**

> Copy all this conversation of user prompts with AI responses into a new `day3sucess.md` file so another coder can use the same prompts and methodology.

**Assistant action**

This standalone file was created from the available user-visible decision journal and extended with this request. It preserves reusable prompts, outcomes, evidence, corrections, and current limitations. It does not claim to contain hidden AI thinking or unavailable verbatim responses.
...
Objective:
Implement an automated, auditable AI-assisted engineering workflow for this project.

Use:
- Coordinating profile: profile_sse
- Required skill: retail-coding-standards
...
Request
  ↓
Policy gate
  - Validate the selected profile, skill, subagent, permissions, and scope.
  - Reject unauthorized operations, agents, paths, commands, and scope expansion.
  - Protect billing.sqlite3 and other declared protected resources.
  ↓
Profile + skill + specialized subagent
  - Apply the project profile’s permissions.
  - Apply retail-coding-standards.
  - Delegate only a concrete, bounded implementation task to feature_implementer.
  ↓
Structured activity and evidence log
  - Record the business outcome and acceptance criteria.
  - Record authorized and protected paths.
  - Record actors, operations, changed files, commands, and actual outcomes.
  - Never record secrets, customer data, private reasoning, or database contents.
  ↓
Tests + policy checks + change review
  - Compare actual Git changes with the declared evidence scope.
  - Reject unrecorded, out-of-scope, or protected-path changes.
  - Run compilation, focused tests, the complete project suite, and policy validation.
  - Review compatibility, security, atomicity, failure handling, and unrelated changes.
  ↓
Human approval
  - Keep approval pending until a human reviews the implementation, diff, and evidence.
  - Do not allow the AI to approve its own work.
  ↓
CI/release gate
  - Require all mandatory checks to pass.
  - Require protected-environment human approval.
  - Block release when checks, evidence, corrective actions, or approval are incomplete.
  - Do not deploy unless deployment is separately configured and explicitly authorized.

Exception and mistake workflow:

Exception or AI mistake detected
  ↓
Record it in the evidence log
  - Category
  - Trigger
  - Business or technical impact
  - Reproduction or review evidence
  ↓
Display it in the governance dashboard
  ↓
Create a corrective action
  - Identify the responsible owner.
  - Correct the implementation or control.
  - Add a regression test when appropriate.
  - Update the relevant profile, skill, policy, or documentation only when evidence
    shows that the update would prevent recurrence.
  ↓
Rerun all affected checks
  ↓
Preserve the original exception and correction in project history.

Implementation requirements:

1. Use only Python standard-library tooling unless a dependency is explicitly justified.
2. Store machine-readable governance policy under governance/.
3. Store one evidence record per change under governance/evidence/.
4. Provide a local policy-validation command.
5. Generate a human-readable governance dashboard.
6. Add automated tests for allowed and rejected operations.
7. Add a CI workflow for policy validation, compilation, tests, dashboard generation,
   human approval, and the release gate.
8. Use temporary databases for tests.
9. Do not modify billing.sqlite3 or run persistent migrations.
10. Do not merge, deploy, or perform destructive operations.
11. Update README.md and governance documentation.
12. Record actual results only; do not mark planned checks as passed.

Required validation:

- Authorized profile, skill, subagent, and scoped operations are accepted.
- Read-only profiles cannot record write operations.
- Unauthorized subagents are rejected.
- Protected-path and path-traversal changes are rejected.
- Actual Git changes must match the current evidence record.
- Unrecorded or out-of-scope files are rejected.
- Required commands cannot be replaced with weaker commands.
- Failed or pending checks block release.
- Unresolved exceptions block release.
- Missing human approval blocks release.
- The full existing application suite continues to pass.
- billing.sqlite3 remains unchanged.

Final report:

- Delivered controls and changed files
- Profile, skill, and subagent used
- Actual validation commands and results
- Recorded exceptions and corrective actions
- Human-approval state
- CI/release-gate state
- Remaining configuration or operational limitations
